        ...
        try:
            ...
            for i in range(0, len(purchase_orders), batch_size):
                ...
                for po_data in batch:
                    ...
                    try:
                        po_filter = {"custom_sage_name": po_name, "company": company.company}
                        is_update = False
                        ...
                        if frappe.db.exists("Purchase Order", po_filter):
                            ...
                            po_doc.company = company.company
            ...
            summary_title = f"Sage PO Summary {company.company}"[:140]
            summary = f"Company: {company.company} | Processed {len(purchase_orders)} POs | Updated: {len(updated_pos)}, Created: {len(created_pos)}, Skipped: {len(skipped_pos)}"
